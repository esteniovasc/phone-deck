...
export function PhoneCard({ data, visualStatus, onEdit, onDelete, onSaveDraft, isReadOnly = false }: PhoneCardProps) {
	...
	return (
		<div className="group relative">
			{/* Main Card Container */}
			<div
				className={`relative bg-white border border-slate-100 rounded-lg shadow-sm hover:shadow-md transition-all overflow-hidden ${data.isMinimized ? 'max-w-xs' : 'max-w-2xl'
					...
					<div className="flex">
						...
						<div className="w-2/3 p-6 flex flex-col justify-between">
							...
							<div className="flex items-center justify-between mb-4">
								{data.isDraft && !isReadOnly ? ( // Só permite editar draft se não estiver em ReadOnly
									<input
									...
									<h2 className="text-lg font-bold text-slate-900">{data.isDraft ? draftName : data.model}</h2> // Mostra draftName se for draft mas readonly
								)}
			...
			{/* Side Action Buttons (Floating Outside) */}
			{/* Use left-full + padding-left to create a safe hover bridge */}
			{/* SOMENTEMOSTRA SE NÃO ESTIVER EM READ ONLY */}
			{!isReadOnly && (
				<div className="absolute left-full top-0 h-full flex flex-col justify-start pt-2 gap-3 pl-4 opacity-0 group-hover:opacity-100 transition-all duration-200 z-20 pointer-events-none group-hover:pointer-events-auto">
					{/* Botão Editar */}
					<button
						onClick={(e) => {
							e.stopPropagation();
						...
						className="p-3 bg-white text-slate-400 hover:text-blue-600 hover:scale-110 shadow-lg rounded-full transition-all border border-slate-100"
					...
					<button
						...
						className="p-3 bg-white text-slate-400 hover:text-red-600 hover:scale-110 shadow-lg rounded-full transition-all border border-slate-100"
